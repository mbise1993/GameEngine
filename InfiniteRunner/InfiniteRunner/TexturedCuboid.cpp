...
TexturedCuboid::TexturedCuboid( const std::string& name, float width, float depth, float height, RGBA faceColor ) :
...
void TexturedCuboid::generate()
{
	...
	{
		gen.textureCoords[ti++] = { 0, 1 };
		gen.textureCoords[ti++] = { 0, 0 };
		gen.textureCoords[ti++] = { 1, 0 };
		gen.textureCoords[ti++] = { 1, 1 };
	}
	...
	index_buffer_t* indexData = gen.generateIndexData();
