...
void OGLReferenceFrame::move( const glm::vec3& direction, float speed )
{
	...
	translateMat[3] = glm::vec4( (direction * speed) / this->scale, 0.0f );
...
void OGLReferenceFrame::translate( float dx, float dy, float dz )
{
	...
	translation[3].x = dx / this->scale;
	translation[3].y = dy / this->scale;
	translation[3].z = dz / this->scale;
...
void OGLReferenceFrame::scaleBy( float amount )
{
	// Create the matrix
	glm::mat4 scaleMat( amount );

	// Reset w
	scaleMat[3] = glm::vec4( 0, 0, 0, 1 );

	// Apply scale
	this->orientation *= scaleMat;
